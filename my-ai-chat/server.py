...
def get_cached_model(model_name: str, dept_id: int, knowledge_base: str, agent_key: str) -> Optional[Any]:
...
def cache_model(model_name: str, dept_id: int, knowledge_base: str, agent_key: str, model_instance: Any, retrieval_chain: Any = None):
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        for kb in knowledge_bases:
            ...
            if not rag.check_embedding(dept_id=str(dept_id), knowledge_base_id=str(kb_id)):
        ...
        for agent in agents:
            try:
                ...
                knowledge_base = config.get("knowledge_base_id")
...
async def process_chat_non_streaming(
    ...
    try:
        ...
        model_instance, retrieval_chain = get_cached_model(model_name, department_id, knowledge_base_id, agent_key)
        if not model_instance:
            ...
            retrieval_chain = model_instance.retrieval(
                ...
                prompt=prompt,
                nftext="ขออภัย ระบบไม่สามารถให้คำตอบได้",
            ...
            cache_model(model_name, department_id, knowledge_base_id, agent_key, model_instance, retrieval_chain)
...
async def get_shared_agents(user = Depends(get_current_user)):
    """
    Get all shared agents for the current user.
    Only returns shared agents owned by the current user.
    """
    try:
        # Get shared agents from database
        shared_agents = db.get_shared_agents(user_id=user["user_id"])
        ...
        formatted_agents = [{
            ...
            "name": agent["name"],
            "description": agent["description"],
            "allowedOrigins": agent["allowed_origins"].split(",") if agent["allowed_origins"] else ["*"],
        ...
        return {"sharedAgents": formatted_agents}
        
    except Exception as e:
        logger.error(f"Error getting shared agents: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=str(e)
        )
...
async def chat_endpoint(request: ChatRequest, user = Depends(get_current_user)):
    ...
    if not request.conversation_id and len(request.messages) == 1:
        ...
        # Get agent configuration before creating conversation
    ...
    if request.stream:
        ...
        async def stream_with_save():
            ...
            # Initial event with agent_msg_id
...
async def delete_knowledge_base(
    ...
    try:
        ...
        result = model.delete_collection(
            dept_id=dept_id,
...
async def create_document(
    ...
    try:
        ...
        if document_create.file_url and validators.url(document_create.file_url):
            ...
            # Use LangChain's WebBaseLoader to fetch content
        ...
        async with embedding_lock:
            ...
            embeddings_exist = model.check_embedding(
                dept_id=str(dept_id),
                knowledge_base_id=str(kb_id)
            )
            ...
            if embeddings_exist:
                ...
                result = model.append_embeddings(
                    dept_id=str(dept_id),
                    docs=docs,
                    knowledge_base_id=str(kb_id)
                )
        ...
        print(f'document_create: {document_create}')    
        ...
        document_id = db.add_document_to_knowledge_base(
            ...
            content=contents,
            file_url=web_url or file_path,
            file_type=file.content_type if file else "web_content" if web_url else "text"
        )
...
async def add_user_to_department(
    ...
    success = db.add_user_to_department(user_id, department_id)
...
async def add_permission_to_user(
    ...
    if not success:
        raise HTTPException(
            status_code=400,
            detail=f"Failed to add permission '{request.permission_name}' to user. Permission may not exist."
        )
