...
export function middleware(request: NextRequest) {
  ...
  const isAllowedOrigin = allowedOrigins[0] === '*' || allowedOrigins.includes(origin);

  // Handle preflighted requests (OPTIONS)
  const isPreflight = request.method === 'OPTIONS';
  if (isPreflight) {
    const preflightHeaders = {
      'Access-Control-Allow-Origin': allowedOrigins[0] === '*' ? '*' : (isAllowedOrigin ? origin : ''),
      ...corsOptions,
    }
  ...
  if (token && (request.nextUrl.pathname === '/login' || request.nextUrl.pathname === '/login/')) {
    console.log('✔️ Redirecting to home page due to existing token on login page');
    let redirectPath = '/';
...
function applyHeaders(response: NextResponse, origin: string, isAllowedOrigin: boolean) {
  response.headers.set('Access-Control-Allow-Origin', allowedOrigins[0] === '*' ? '*' : (isAllowedOrigin ? origin : ''));
