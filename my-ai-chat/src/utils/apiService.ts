...
// API Base URL - adjust based on your backend server location
...
/**
 * Login user - this is now redundant but kept for API compatibility
 * Applications should use authService.login() instead
 */
...
export const getAgents = async (): Promise<Agent[]> => {
  try {
    ...
    if (userProfile?.UserRole !== 'Admin' && userProfile?.departments?.length > 0) {
    ...
    return data.map((agent: any) => ({
      ...
      model: agent.configuration ? JSON.parse(agent.configuration).model || "gpt-3.5-turbo" : "gpt-3.5-turbo",
...
export const createDocumentWithFile = async (
  ...
  try {
    ...
    // Create a DocumentCreate model to align with server expectations
    const documentData = {
    ...
    // Append JSON document data as a string in a field named 'document'
...
export const shareAgent = async (agentId: number, data: SharedAgentData): Promise<SharedAgent> => {
  try {
    const expiresAt = data.expiryDays ? new Date(Date.now() + data.expiryDays * 86400000).toISOString() : null;
...
export const getSharedAgents = async (): Promise<SharedAgent[]> => {
  try {
    ...
    return data.sharedAgents.map((agent: any) => ({
      id: agent.id,
      apiKey: agent.apiKey,
      agentId: parseInt(agent.agentId),
      name: agent.name,
      description: agent.description || '',
      allowedOrigins: typeof agent.allowedOrigins === 'string' ? agent.allowedOrigins.split(',') : agent.allowedOrigins || ['*'],
...
export const checkUserPermission = async (permissionName: string, userId?: number): Promise<boolean> => {
  try {
    let url = `/api/permissions/check/${permissionName}`;
