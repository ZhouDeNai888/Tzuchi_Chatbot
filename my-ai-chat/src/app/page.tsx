...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated && !isLoading) {
      console.log(
        "Not authenticated and loading complete - redirecting to login page..."
      );
        ...
        setAgents([
          {
            ...
            description:
              "General knowledge, daily tasks, and basic information",
            model: "gpt-4o-mini",
            temperature: 0.7,
            max_tokens: 2000,
            system_prompt: "",
          ...
          },
          {
            ...
            name: "Code Expert",
            description:
              "Programming, software development, and technical solutions",
            model: "gpt-4",
            temperature: 0.7,
            max_tokens: 4000,
            system_prompt: "",
  ...
  const handleFeedback = async (
    ...
    try {
      if (newFeedback) {
        // Only submit if we have feedback (not undefined)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // If agent changed (by agent_key), generate a new chat_id
      let chatId = sessionStorage.getItem("chat_id");
      const lastAgentKey = sessionStorage.getItem("chat_agent_key");
      if (!chatId || lastAgentKey !== agent?.agent_key) {
        chatId = generateUID(); // Generate a new UID if no chat_id exists or agent changed
        sessionStorage.setItem("chat_id", chatId); // Store the new chat_id in sessionStorage
        sessionStorage.setItem("chat_agent_key", agent?.agent_key || ""); // Track current agent_key
      }
        ...
        <div className="flex-1 pt-16">
          ...
          >
            <div className="flex justify-between items-center">
              <span>
                {agents.length > 0
                  ? getLocalizedAgents().find(
                      (a) => String(a.id) === selectedAgent
                    )?.name
                  : "No agents"}
              </span>
              <svg
      ...
      <div className="flex-1 flex flex-col max-w-4xl mx-auto w-full pb-24">
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.map((message, index) => (
            ...
            >
              <div
                className={`max-w-[70%] rounded-lg p-3 break-words shadow-sm ${
                  message.isUser
                  ...
                  <ReactMarkdown
                    ...
                    components={{
                      ...
                      pre: ({ node, children, ...props }) => (
                        <pre
                          className="bg-gray-900 p-2 rounded my-2"
                          {...props}
                        >
                          {children}
                        </pre>
                      ...
                      table: ({ node, children, ...props }) => (
                        <table
                          className="border-collapse my-4 w-full"
                          {...props}
                        >
                          {children}
                        </table>
                      ...
                      th: ({ node, children, ...props }) => (
                        <th
                          className="border border-gray-300 dark:border-gray-600 px-4 py-2 font-bold text-left"
                          {...props}
                        >
                          {children}
                        </th>
                ...
                {!message.isUser && (
                  <div className="flex justify-end mt-2 space-x-2">
                    ...
                    >
                      <i
                        className={`fa-${
                          message.feedback === "like" ? "solid" : "regular"
                        } fa-thumbs-up`}
                      ></i>
                    </button>
                    <button
                    ...
                    >
                      <i
                        className={`fa-${
                          message.feedback === "dislike" ? "solid" : "regular"
                        } fa-thumbs-down`}
                      ></i>
                    </button>
                    <button
                    ...
                    >
                      <i
                        className={`fa-regular fa-${
                          copyStatus[message.id!] ? "check" : "copy"
                        }`}
                      ></i>
                    </button>
                  </div>
