...
export default function Knowledge() {
  ...
  const [knowledgeToDelete, setKnowledgeToDelete] = useState<number | null>(
    null
  );
  // Pagination states
  ...
  useEffect(() => {
    const fetchKnowledgeBases = async () => {
      ...
      try {
        ...
        console.log(
          "Fetching knowledge bases for department ID:",
          departmentId
        );
  ...
  const handleCreateKnowledge = async (title: string, description: string) => {
    ...
    const titleExists = knowledgeBases.some(
      (kb) =>
        kb.title && title && kb.title.toLowerCase() === title.toLowerCase()
    );
    ...
    if (!isEnglishOnly(title)) {
      ...
      alert(
        t.englishTitleOnly ||
          "Please enter title in English only / 請使用英文輸入標題"
      );
      return;
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setKnowledgeBases((prev) =>
        prev.filter((kb) => kb.id !== knowledgeToDelete)
      );
    } catch (err: any) {
  ...
  const getKnowledgeUrl = (kb: KnowledgeBase) => {
    ...
    // Generate slug: replace spaces with "-", remove special chars except Unicode letters/numbers
    const slug = kb.title
      .toLowerCase()
      .replace(/\s+/g, "-")
      .replace(/[^\p{L}\p{N}-]/gu, "");
  ...
  const paginatedItems = existingItems.slice(
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {newItems.map((kb) => (
                  <Link
                    ...
                    className={`rounded-lg p-6 hover:bg-opacity-90 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-xl cursor-pointer block relative group ${getGradientClass(
                      kb.id
                    )}`}
                  >
