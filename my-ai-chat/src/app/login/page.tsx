...
export default function Login() {
    ...
    const handleSubmitRequest = async (e: React.FormEvent) => {
        ...
        if (!email.trim() || !feedback.trim()) {
            setModalMessage({
                type: 'error',
                text: language === 'en' ? 'Please fill out all fields' : '請填寫所有欄位'
            });
            return;
        }
        ...
        if (!emailRegex.test(email)) {
            setModalMessage({
                type: 'error',
                text: language === 'en' ? 'Please enter a valid email address' : '請輸入有效的電子郵件地址'
            });
            return;
        }
    ...
    const handleForgotPasswordSubmit = async (e: React.FormEvent) => {
        ...
        if (!forgotPasswordEmail.trim()) {
            setForgotPasswordMessage({
                type: 'error',
                text: language === 'en' ? 'Please enter your email address' : '請輸入您的電子郵件地址'
            });
            return;
        }
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 px-4 py-12 sm:px-6 lg:px-8">
            <div className="w-full max-w-md p-8 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl space-y-8 transform transition-all border border-gray-200 dark:border-gray-700">
                <div className="flex flex-col items-center">
                    <div className="mb-8 relative">
                        <div className="absolute -z-10 w-32 h-32 bg-blue-100 dark:bg-blue-900/30 rounded-full blur-xl opacity-70 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"></div>
                        <Image
                            src="/logo.png"
                            alt="Logo"
                            width={160}
                ...
                <form onSubmit={handleSubmit} className="mt-8 space-y-6">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="pl-10 w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-all"
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center">
                            <input
                                ...
                                disabled={isLoadingLocal}
                            />
                            <label htmlFor="remember_me" className="ml-2 block text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
                                {language === 'en' ? 'Remember me' : '記住我'}
                            </label>
                        </div>
                        <div className="text-sm">
                            <a href="#" onClick={handleForgotPasswordClick} className="font-medium text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 hover:underline transition-colors">
                    ...
                    <button
                        type="submit"
                        disabled={isLoadingLocal}
                        className={`group relative w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-md text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors ${isLoadingLocal ? 'opacity-70 cursor-not-allowed' : ''
                            } transform hover:translate-y-[-1px] active:translate-y-[1px] transition-transform`}
                    >
                        <span className="absolute left-0 inset-y-0 flex items-center pl-3">
                            <svg className={`h-5 w-5 text-blue-300 group-hover:text-blue-200 transition-colors ${isLoadingLocal ? 'animate-spin' : ''}`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                            </svg>
                        </span>
                        {isLoadingLocal
                ...
                <div className="mt-8">
                    ...
                    <div className="mt-6">
                        <p className="text-center text-sm text-gray-600 dark:text-gray-400">
                            ...
                            <a href="#" onClick={handleContactAdminClick} className="font-medium text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 hover:underline transition-colors">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full p-6 relative">
                        ...
                        {modalMessage && (
                            <div className={`p-4 mb-4 rounded-lg ${modalMessage.type === 'success'
                        ...
                        <form onSubmit={handleSubmitRequest} className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-all"
                                    placeholder={language === 'en' ? 'your.email@example.com' : '您的郵箱@example.com'}
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    rows={4}
                                    className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-all"
                                    placeholder={language === 'en' ? 'Describe why you need an account or any other assistance...' : '描述您為什麼需要帳戶或任何其他協助...'}
                                    disabled={isSubmitting}
                                />
                            </div>
                            ...
                            <div className="pt-2">
                                <button
                                    type="submit"
                                    disabled={isSubmitting}
                                    className={`w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-md text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''
                                        }`}
                                >
                                    {isSubmitting ? (
                                        <>
                                        ...
                                        language === 'en' ? 'Submit Request' : '提交請求'
                                    )}
                                </button>
                            </div>
                        </form>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full p-6 relative">
                        ...
                        {forgotPasswordMessage && (
                            <div className={`p-4 mb-4 rounded-lg ${forgotPasswordMessage.type === 'success'
                                ? 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-800'
                                : 'bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 border border-red-200 dark:border-red-800'
                                }`}>
                        ...
                        <form onSubmit={handleForgotPasswordSubmit} className="space-y-4">
                            ...
                            <div className="pt-2">
                                <button
                                    type="submit"
                                    disabled={isForgotPasswordSubmitting}
                                    className={`w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-md text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors ${isForgotPasswordSubmitting ? 'opacity-70 cursor-not-allowed' : ''
                                        }`}
                                >
                                    {isForgotPasswordSubmitting ? (
                                        ...
                                        language === 'en' ? 'Send Reset Instructions' : '發送重設說明'
                                    )}
                                </button>
                            </div>
                        </form>
